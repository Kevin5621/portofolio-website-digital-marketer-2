    ...
    <section ref={(el) => registerSection(el as HTMLDivElement | null, 1)} className="py-24 px-6">
        <div className="max-w-6xl mx-auto">
            <div className="mb-16 grid grid-cols-1 md:grid-cols-12 gap-12">
                ...
                <p className="text-red-500 uppercase tracking-widest mb-3 text-sm">Expertise</p>
                <h2 className="text-4xl font-light text-stone-900 mb-6 relative">
                    Skills Applied
                    <span className={`block absolute -bottom-2 left-0 h-px bg-red-500 transition-all duration-700 delay-300 ${sectionViewed[1] ? 'opacity-100 w-16' : 'opacity-0 w-0'}`}></span>
                </h2>
                </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-0.5">
                ...
                >
                    <div className={`absolute inset-0 bg-red-500 transition-all duration-0 ${hoveredSkill === index ? 'opacity-100' : 'opacity-0'}`}></div>
